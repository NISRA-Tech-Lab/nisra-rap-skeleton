...
* Ensure colour contrast ratios between adjacent chart elements and against background is min 3:1
...
```{r fig6d, echo = TRUE, fig.alt="Figure 6d alt text example. More detail on the trends are included in the text directly above or below the chart, and data can be downloaded using the button below", out.width="100%"}
...
##### NI population in 2022 in 5 age bands

```{r fig7, echo = TRUE, fig.alt="Figure 6e alt text example. More detail on the trends are included in the text directly above or below the chart, and data can be downloaded using the button below", out.width="100%"}
...
fig7 <- plot_ly(df_mye_latest_5_agegroups,
                 ...
                 y = ~agegroup_total,
                 type = "bar",
                 marker = list(color = c(nisra_blue, nisra_navy, 
                                         nisra_col3_green, nisra_col4_purple, nisra_col5_lilac)),
                 # order of colours is important: need medium and dark colours beside one another
                 text = ~format(agegroup_total, big.mark = ","),
                 textfont = list(color = c("white", "white",
                                           "black", "white", "black")),
                 # need to specify black or white labels. plotly default is grey and fails contrast required
                 textangle = "horizontal",
                 hovertemplate = ~paste0(age_group, ": ", format(agegroup_total, big.mark = ","),
                                         "<extra></extra>"),
                hoverlabel = list(font = list(color = c("white", "white", "black", "white", "black")))) %>% 
  layout(yaxis = list(title = "",
                      tickformat = ",d"), # adds thousand separator
         xaxis = list(title = "Age Group"),
         annotations = list(yaxistitle))
...
f_make_tables(df_fig7_xls,
  title = paste0(
    "Figure 7: NI population by 5 age bands, ",
    latest_year
  ),
  data_style = ns_comma,
  data_dir = paste0(here(), "/code/demo/demo_outputs/", "figdata/")
...
#### Figure 8: Higher percentage of females than males in the over 65 age group in
##### Percentage of NI population aged 65 plus by gender, `r latest_year`

```{r piechart, echo = TRUE, fig.alt="Figure 8 alt text example. More detail on the trends are included in the text directly above or below the chart, and data can be downloaded using the button below"}
fig8a <- plot_ly(df_mye_latest_over_under_65,
  ...
  textinfo = "text",
  hoverinfo = "text",
  hoverlabel = list(font = list(color = c("black", "white"))), # hover font colour for contrast
  marker = list(
...
fig8b <- plot_ly(df_mye_latest_over_under_65,
  ...
  marker = list(
    colors = c(nisra_col3_green, nisra_navy)),
  textfont = list(color = c("black", "white")), #plotly default is grey which is insufficient contrast
  showlegend = FALSE
) %>%
  config(displayModeBar = FALSE) %>%
  layout(
...
f_make_tables(df_mye_latest_over_under_65,
  title = paste0(
    "Figure 8: NI population under 65 and 65 plus by gender ",
    latest_year
  ),
  data_style = ns_comma,
  data_dir = paste0(here(), "/code/demo/demo_outputs/", "figdata/")
...
fig9 <- plot_ly(df_fig9,
  ...
  textfont = list(family = "Arial", size = 16, color = c("white", "white", "black", "white", "black"))
...
f_make_tables(df_fig9_xls,
  title = paste0(
    "Figure 9: Female population of NI by age group, ",
    latest_year
  ),
  data_style = ns_comma,
  data_dir = paste0(here(), "/code/demo/demo_outputs/", "figdata/")
...
To demonstrate a more complex tree map with three levels of nesting and multiple tiles, a tree map (Figure 3) from the Economic and Labour Market Statistics report [Structure and Performance of the NI Economy 2018 and 2019](https://datavis.nisra.gov.uk/economy-and-labour-market/structure-of-the-ni-economy-2019.html) is shown below. These colours have not been checked for accessibility.

#### Figure 10: Split of NI GVA by industry, `r df_sut_report_data$Value[1]` 
...
f_make_tables(
  data = fig9_dl,
  title = paste0(
    "Figure 10: Split of NI GVA by industry, ",
...
fig11 <- plot_ly(df_mye_latest_year_agegroup,
  ...
  marker = list(
    ...
    line = list(color = "black", width = 1) # border required to ensure contrast between nisra blue and lilac
  ),
...
```{r figure11_dl_buttons}
f_make_tables(fig11_xls,
  paste0("Figure 11: NI population by age group ", latest_year),
  data_style = ns_comma,
  data_dir = paste0(here(), "/code/demo/demo_outputs/", "figdata/")
)
```
